...
/*
 Sonnenstunden-Diagramm letzer 10 Tage
 ================================================
 */
socket.on('suntime', function (data) {
    ...
    var svg = d3.select("#suntimeTenDays").append("svg")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
        .append("g")
        .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    ...
    var tip = d3.tip()
        ...
        .offset([-10, 0])
        .html(function(d) {
            return "<strong>Sonnenstunden:  </strong> <span style='color:red'>" +  Number((d.suntime).toFixed(1));  + "</span>";
        });
